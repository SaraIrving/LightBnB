...
const addUser =  function(user) {
  ...
  .then(res => res.rows[0])
  .catch('There was a problem while attempting to add this user!');
}
...
const getAllReservations = function(guest_id, limit = 10) {
  return pool.query (
    ...
    .then(res => res.rows)
    .catch('We can\'t find any past reservations!');
}
...
const getAllProperties = function(options, limit = 10) {
  ...
  `;
   ...
   .then(res => res.rows)
   .catch('We can\'t find any properties!');
}
...
const addProperty = function(property) {
  ...
  .then(res => res.rows[0])
  .catch(console.log('Something has gone wrong when we tried to add your property. Sorry!'));
}
